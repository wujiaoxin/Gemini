...
class risk extends Admin {
	...
	public function ratingInfo($id = null) {
		...
		if (IS_POST) {
			...
			$creditList = db('credit')->alias('c')->field('c.*,m.realname,m.idcard,o.loan_limit')->join('__MEMBER__ m','c.uid = m.uid')->join('__ORDER__ o','c.order_id = o.id')->where("c.id",$id)->order('id desc')->fetchSQL(false)->find();			
			$data = array(
				'infoStr' =>json_encode($creditList),
			);			
			$this->assign($data);
